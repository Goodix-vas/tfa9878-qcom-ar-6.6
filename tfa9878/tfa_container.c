 ...
 */
static struct tfa_desc_ptr *tfa_cnt_get_dsc(struct tfa_container *cnt,
 ...
 */
static enum tfa98xx_error tfa_run_write_register(struct tfa_device *tfa,
	...
	struct tfa_msg *msg, char *buffer, int *size)
{
	...
	for (i = 0; i < msg->msg_size; i++) {
		buffer[nr++] = (uint8_t)((msg->data[i] >> 16) & 0xff);
...
enum tfa98xx_error tfa_cont_write_files(struct tfa_device *tfa)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dsc_cmd) {
			...
			pcmd = dev->list[i].offset
				+ (char *)tfa->cnt;
			size = *(uint16_t *)pcmd;
			err = dsp_msg(tfa, size, pcmd + 2);

			if (tfa->verbose) {
				const char *cmd_id = pcmd + 2;
...
static enum tfa98xx_error tfa_cont_write_item(struct tfa_device *tfa,
	...
	int length, const char *buffer)
{
	...
	len_word_in_bytes = (tfa->convert_dsp32) ? 4 : 3;
